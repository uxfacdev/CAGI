...
tsv_path   = r"C:\Users\Kunny\Research\Project\BiConVarNet\ATP7B\ATP7B_variants_formatted.tsv"
pdb_dir    = r"E:\CAGI_data\pdb_files"
output_dir = r"E:\CAGI_data\voxel_cache_ATP7B_noRSA"
os.makedirs(output_dir, exist_ok=True)

def voxelize_and_save(args):
    uid, wt1, mut1, structure_file, mut_pos = args
    wt1  = str(wt1).strip().upper()
    mut1 = str(mut1).strip().upper()
    mut_pos = int(mut_pos)
    ...
    try:
        feat, mapping, qc = compute_voxel_features_structural(
            pdb_path, mut_pos,
            chain_id=None,                 
            add_dpos2=True, add_plddt=True, add_rsa=False, add_resdepth=False, add_gnm_msf=True,
            wt1=wt1                           
        )
...
if __name__ == "__main__":
    ...
    rows = list(zip(df["UniProtID"], df["WT"], df["Mut"], df["StructureFile"], df["MutPos(pdb)"]))

    results, qc_rows = [], []
    with ProcessPoolExecutor(max_workers=16) as ex:
        
        for key, status, qc_row in tqdm(ex.map(voxelize_and_save, rows, chunksize=10), total=len(rows)):
