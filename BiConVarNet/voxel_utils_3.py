...
def _rsa_map_biopython(pdb_path: str, chain_id: Optional[str] = None) -> Optional[Dict[int,float]]:
    try:
        from Bio.PDB import PDBParser, ShrakeRupley
        parser = PDBParser(QUIET=True)
        structure = parser.get_structure("x", pdb_path)
        sr = ShrakeRupley(); sr.compute(structure, level="R")
        rsa_per_chain: Dict[str, Dict[int, float]] = {}
        for model in structure:
            for chain in model:
                c = chain.id
                m = rsa_per_chain.setdefault(c, {})
                for res in chain:
                    hetflag, resnum, icode = res.id
                    if hetflag != " ": continue
                    asa = float(res.xtra.get("EXP_ASA", 0.0))
                    m[resnum] = asa
        if not rsa_per_chain: return None
        if chain_id is not None and chain_id in rsa_per_chain:
            chosen = rsa_per_chain[chain_id]
            arr = np.array(list(chosen.values()), dtype=np.float32)
            scale = float(np.percentile(arr, 95)) if arr.size else 1.0
            scale = scale if scale > 1e-6 else 1.0
            return {k: float(min(max(v/scale, 0.0), 1.0)) for k, v in chosen.items()}
        first = next(iter(rsa_per_chain))
        arr = np.array(list(rsa_per_chain[first].values()), dtype=np.float32)
        scale = float(np.percentile(arr, 95)) if arr.size else 1.0
        scale = scale if scale > 1e-6 else 1.0
        return {k: float(min(max(v/scale, 0.0), 1.0)) for k, v in rsa_per_chain[first].items()}
    except Exception:
        return None
...
def compute_voxel_features_structural(
    ...
    if add_rsa or add_resdepth:
        rsa_by_resnum = _rsa_map_biopython(pdb_path, chain_id=chain_id)
    if add_rsa:
