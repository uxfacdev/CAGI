...
df_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\S669_Offset_Corrected.tsv"
fail_log_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\S669_failed_fasta_match.tsv"
fasta_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\S669.fasta"
...
def check_row(row):
    # 기본 정보 추출
    uid = row["PDB"]
    structure_file = row["PDB_ID_FINAL"][:-1] + ".pdb"
    mut_pos = int(row["POS"])-1
    mut_pos_pdb = int(row["MAPPED_PDB_POS"]) 
    wt = row["WT"]
    chain = str(row["PDB_ID_FINAL"][-1])
