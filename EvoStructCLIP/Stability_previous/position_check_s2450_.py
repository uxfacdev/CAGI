...
df_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\S2450_Final_PDB_Mapped.tsv"
fail_log_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\S2450_failed_fasta_match.tsv"
...
def check_row(row):
    # 기본 정보 추출
    uid = row["PDB"]
    structure_file = row["MAPPED_PDB"] + ".pdb"
    mut_pos = int(row["POS"])-1
    mut_pos_pdb = int(row["MAPPED_PDB_POS"]) 
    wt = row["WT"]
    chain = str(row["MAPPED_CHAIN"])
