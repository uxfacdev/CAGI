...
# 경로 설정
from pathlib import Path  # 상단에 추가되어 있어야 함

# 경로 설정 부분 수정
pdb_dir = Path(r"C:\Users\Kunny\Downloads\PROSTATA-main\PDB")
df_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\PROSTATA_formatted_clean.csv"
fail_log_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\PROSTATA_failed_fasta_match.tsv"
fasta_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\PROSTATA.fasta"

clean_df_path = r"C:\Users\Kunny\Documents\GitHub\CAGI\EvoStructCLIP\Stability\PROSTATA_formatted_clean.csv"
...
def check_row(row):
    ...
    pdb_full_id = pdb_full_id.replace('.pdb', '')
    structure_file = pdb_dir / f"{pdb_full_id}.pdb" # 전체 ID 사용
    if not structure_file.exists():
        structure_file = pdb_dir / f"{pdb_full_id[:4].upper()}.pdb"

    if len(pdb_full_id) >= 5 and "_" not in pdb_full_id:
        chain = pdb_full_id[-1]
    else:
        chain = 'A' # 기본값
        
    mut_pos = int(row["POS"])-1
    mut_pos_pdb = int(row["MAPPED_PDB_POS"]) 
    wt = row["WT"]
    ...
    if sequence:
        try:
            ...
            if mut_pos < len(sequence):
                fasta_match = (sequence[mut_pos] == wt)
                if not fasta_match:
...
if __name__ == "__main__":
    ...
    # keep_mask = [not r["is_failed"] for r in results]
    # df_clean = df[keep_mask].copy()
    # df_clean.to_csv(clean_df_path, sep="\t", index=False)
    # print(f"정제된 데이터 저장 완료: {clean_df_path}")
